...
void CommandHandler::onJsonCommand(const std::string& jsonStr) {
    ...
    switch (msg.cmdType) {
        ...
        case CmdType::CTRL_SLOT_GET_PARAM:    handleCtrlSlotGetParam(payload);    break;
...
// -----------------------------------------------------------------------------
void CommandHandler::onBinaryCommand(const std::vector<uint8_t>& binData) {
    if (binData.empty()) {
        DBG_PRINTLN("[CMD] Empty binary command");
        return;
    }
    ...
    if (!result.valid) {
        DBG_PRINTF("[CMD] Invalid binary command opcode=0x%02X\n", opcode);
        return;
    }
    ...
    case BinaryCommands::Opcode::SET_VEL:
        DBG_PRINTF("[CMD BIN] SET_VEL vx=%.3f omega=%.3f\n",
    ...
    case BinaryCommands::Opcode::HEARTBEAT:
        // Already handled by mode_.onHostHeartbeat() above
        break;

    case BinaryCommands::Opcode::STOP:
        DBG_PRINTLN("[CMD BIN] STOP");
        motion_.stop();
        break;
    }
}
...
void CommandHandler::handleCtrlSlotGetParam(JsonVariantConst payload) {
